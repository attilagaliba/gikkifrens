...
const FlowingBalance: React.FC<{
  ...
  flowRate: bigint;
  degenPrice: number;
}> = memo(({ startingBalance, startingBalanceDate, flowRate, degenPrice }) => {
  ...
  return decimalPlaces !== undefined
    ? (
        Number(toFixedUsingString(formatEther(flowingBalance), decimalPlaces)) *
        degenPrice
      ).toFixed(6)
    : formatEther(flowingBalance);
});
FlowingBalance.displayName = "FlowingBalance"; // Assigning displayName
